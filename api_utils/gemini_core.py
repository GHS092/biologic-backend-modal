...
async def run_clinical_analysis(
    ...
    try:
        ...
        prompt_text = (
            ...
            "9. LEY DE LA ASIMETRÍA ORGÁNICA BILATERAL: Antes de emitir un diagnóstico basado en una sola estructura central, el sistema debe realizar un barrido de densidad y volumen en las estructuras bilaterales/espejo (ej. riñones, pulmones). Si existe una distorsión grosera de la arquitectura o un gradiente de densidad anómalo en un órgano par en comparación con su contralateral, la anomalía del órgano par debe integrarse en la cadena causal como posible lesión primaria.\n"
            "10. LEY DEL EFECTO DE MASA VS. DILATACIÓN LUMINAL INTRÍNSECA: Cuando se identifique un tejido hipodenso/isodenso rodeando una estructura vascular o tubular, el sistema debe evaluar si los límites externos de dicho tejido respetan la morfología geométrica del vaso (circular regular) o si presentan contornos lobulados/invasivos hacia el espacio intersticial. Si los contornos externos son irregulares o lobulados, debe clasificarse primariamente como \"Masa extrínseca/Infiltración\" y no como patología intraluminal (trombo/aneurisma).\n"
            "11. LEY DE VALIDACIÓN DE ESTRUCTURAS DE ALTA DENSIDAD (Prótesis/Artefactos): Cualquier estructura circular con una densidad cercana a la ósea o metálica en el trayecto de un conducto debe evaluarse bajo la hipótesis de \"material protésico o calcificación severa\". El sistema tiene prohibido asumir que el diámetro total de la lesión se limita al vaso modificado sin antes descartar que el tejido circundante sea de naturaleza celular diferente.\n"
        )
        ...
        if attached_files and len(attached_files) > 0:
            prompt_text += (
                ...
                "PASO 3 (Análisis Visual y Fenotipado Profundo): Analiza las imágenes. Detalla anomalías en 'diagnosticFindings'. Aplica estrictamente estas fases espaciales:\n"
                "  - FASE DE SEGMENTACIÓN ABSTRACTA: Obligatorio listar en este orden: 1. Estructuras Centrales. 2. Estructuras Pares/Bilaterales. 3. Espacios Virtuales (Retroperitoneo/Pleura).\n"
                "  - FASE DE CAUSALIDAD: Si hay masa en un órgano sólido par y tejido blando envolviendo un conducto central, la masa del órgano sólido asume jerarquía de lesión primaria.\n"
        ...
        if is_debate_mode:
            ...
            try:
                ...
                debate_prompt = (
                    ...
                    "11. Sub-regla Censor de Alucinaciones Anatómicas: Tu función NO es complementar el diagnóstico del agente principal, sino intentar DESTRUIRLO. Busca diagnósticos diferenciales alternativos basados en las estructuras bilaterales o espacios virtuales que el agente principal ignoró por anclarse prematuramente en la geometría de un solo conducto central.\n\n"
